          ...
          <v-card-text>
            <v-container>
              <form @submit.prevent="onSignup">
                ...
                <v-layout row>
                  <v-flex xs12>
                    <v-text-field name="password" label="Password" id="password" v-model="password" type="password" required :rules="[comparePasswords]">
                    </v-text-field>
                  </v-flex>
                </v-layout>
                <v-layout row>
...
<script>
  export default {
    ...
    methods: {
      onSignup () {
        this.$store.dipatch('signUserUp', {email: this.email, password: this.password})
      },
      onDismissed () {
