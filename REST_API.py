...
@app.route('/api/register_image', methods=['POST'])
def register_image():
    logger.info("register_image  start...")
    logger.info(request.json)

    if not request.json['name'] or not request.json['dataURLs']:
        result = {'result': '0', 'msg': "Parameter error"}
    else:
        strImgs = request.json['dataURLs']
        for index, strImg in enumerate(strImgs):
            logger.info('======================================')
            logger.info(strImg)
            recImg = usolUtil.bas64ToRGB(strImg)
            basename = "image"
            suffix = datetime.datetime.now().strftime("%y%m%d_%H%M%S") + str(index)
            filetype = ".jpg"
            img_name = "_".join([basename, suffix]) + filetype
...
if __name__ == '__main__':
    app.run(host='172.17.2.35', debug=False)
